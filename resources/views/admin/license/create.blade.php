                                            ...
                                            <div class="col-lg-8 fv-row">
                                                <select name="user_id" aria-label="{{ __('Select a Customer') }}"
                                                    ...
                                                    @foreach($customers as $key => $value)
                                                        <option value="{{ $value->id }}" {{ (int)old('user_id') === $value->id ? 'selected' :'' }}>{{ $value->full_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <!--end::Col-->
                                            ...
                                            <div class="col-lg-8 fv-row">
                                                <select name="product_id" aria-label="{{ __('Select a Product') }}"
                                                    ...
                                                    @foreach($products as $key => $value)
                                                        <option value="{{ $value->id }}" {{ (int)old('product_id') === $value->id ? 'selected' :'' }}>{{ $value->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <!--end::Col-->
                                            ...
                                            <div class="col-lg-8 fv-row">
                                                <select name="license_type"
                                                    ...
                                                    @foreach($licenseTypes as $value)
                                                        <option value="{{ $value }}" {{ old('license_type') === $value ? 'selected' :'' }}>{{ $value }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <!--end::Col-->
                                            ...
                                            <div class="col-lg-8 fv-row">
                                                <select name="status" aria-label="{{ __('Select...') }}"
                                                    ...
                                                    @foreach($statuses as $key => $val)
                                                        <option value="{{ $key }}" {{ (int)old('status') === $val ? 'selected' :'' }}>{{ __($key) }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <!--end::Col-->
                                        ...
                                        <div class="row mb-6 expiry">
                                            ...
                                            <div class="col-lg-8 fv-row">
                                                <input class="form-control form-control-solid"
                                                       placeholder="{{__('Pick a date')}}" type="date" name="expiry"/>
                                            </div>
                                            <!--end::Col-->
                                        </div>
                                    </div>
...
@push('scripts')
    ...
    <script>
        $("#ltype").change(function () {
            if ($(this).find(":selected").val() == "neomejeno") {
                $(".expiry").hide();
            } else {
                $(".expiry").show();
            }
        ...
        $(e).flatpickr({altInput: !0, altFormat: "F j, Y", dateFormat: "Y-m-d", locale: "sl"})
    </script>
@endpush
